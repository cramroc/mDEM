...
class CoVWeightingLoss(nn.modules.Module):
    ...
    def __init__(self, device, b_train, num_losses, b_mean_decay):
        super(CoVWeightingLoss, self).__init__()
        self.device = device
        # self.n = 4
        self.train = b_train
        self.num_losses = num_losses
        self.mean_decay = b_mean_decay
        self.mean_decay_param = 1.0

        self.current_iter = -1
        self.alphas = torch.zeros((self.num_losses,), requires_grad=True).type(torch.FloatTensor).to(self.device)

        # Initialize all running statistics at 0.
        self.running_mean_L = torch.zeros((self.num_losses,), requires_grad=True).type(torch.FloatTensor).to(
            self.device)
        self.running_mean_l = torch.zeros((self.num_losses,), requires_grad=True).type(torch.FloatTensor).to(
            self.device)
        self.running_S_l = torch.zeros((self.num_losses,), requires_grad=True).type(torch.FloatTensor).to(
            self.device)
    ...
    def forward(self, unweighted_losses):
        ...
        if self.current_iter <= 1:
            self.alphas = torch.ones((self.num_losses,), requires_grad=True).type(torch.FloatTensor).to(
        ...
        self.running_S_l = self.running_S_l + (x_l - self.running_mean_l) * (x_l - new_mean_l)
