...
# Return points that are in circle
def checkIfInCircle(x,r,c):
    """
    x = Coordinates we don't know if are in circle or not (array of floats)
    r = Radius of circle (float)
    c = Coordinates of centre of circle (Array of float)
    """
    # Get distance from coordinates to centre
    distances = getDistance(x, c)
    # Get index of coordinates that are in circle (or within 0.005)
    idx = np.where(distances>(r+0.005))[0]
    # Return coordinates that are in circle
    return x[idx,:]
...
def sampleCircle(r,c,n):
    """
    r = Radius of circle (float)
    c = Coordinates of centre (array of floats)
    n = Number of sample points on circumference (Array of int)
    """
    # Compute a certain number of angles of circle
    theta = np.linspace(0,2*np.pi,n)
    # Compute x & y coordinates for points in circumference of circle
    x = c[0] + r*np.cos(theta)
    y = c[1] + r*np.sin(theta)

    # Concatenate x & y coordinates of circumference points into a single variable
    p = np.concatenate((x.reshape(-1,1), y.reshape(-1,1)),axis=1)

    # Initialise normals
    normals = np.zeros((n,2))
    # Calculate normals for every point of circumference
    for i in range(n):
        # Calculate vector distance from centre to point
        n = c-p[i,:]
        # Normalise vector distance
        n = n/np.linalg.norm(n)

        # Concatenate vector distance into an array
        normals[i,:] = n

    # Return points in circumference and normals
    return p,normals


def getDistancesAndValues(x):
    """
    x = Concatenation of coordinates of circle circumference points and domain points in circle
    """
    # State number of points per edge
    n = 5000

    # Create boundary points for edge
