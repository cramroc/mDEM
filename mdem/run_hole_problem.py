...
import pandas as pd
from CoV.train_options import TrainOptions
from CoV import covweighting_method
...
mu = E / (2 * (1 + nu))  # Lame's first parameter
lam = (E * nu) / ((1 + nu) * (1 - 2 * nu))  # Lame's second parameter


class MultiLayerNet(torch.nn.Module):
    """
    Simple Deep Neural Network architecture
    """
...
def NeoHookean2D(u, x):
    """
    Strain calculations following the Neo-Hookean hyperelastic deformation model
    """
    # Lames parameters
    mu = E / (2 * (1 + nu))
    lam = (E * nu) / ((1 + nu) * (1 - 2 * nu))

    # gradients of displacement (u)
    ...
    # trace of right Cauchy-Green deformation tensor (C = F^T * F)
    trC = Fxx1 ** 2 + Fxy1 ** 2 + Fyx1 ** 2 + Fyy1 ** 2

    # Neo-Hookean deformation model strain energy
    strainEnergy = 0.5 * lam * (torch.log(detF) * torch.log(detF)) - mu * torch.log(detF) + 0.5 * mu * (trC - 2)

    # Sum strain energy to obtain potential energy
...
class mDeepEnergyMethod:
    """
    mixed Deep Energy Method class
    """
    ...
    def __init__(self, inp, out, activation, num_hidden_units, num_layers, dim):
        ...
        # CoV model
        if CoVFlag:
            self.covm = covweighting_method.CoVWeightingMethod(device=dev, mode='train', epochs=iterationADAM,
                                                               adjust_lr=True, lr_mode='plateau', learning_rate=lrADAM,
                                                               num_losses=2, b_mean_decay=False)

    # # ...
    # def train_modelLFBGS(self, data, neumannBC, dirichletBC, delaunayIntegration, tractionFreeCircle, iteration,
    #                      learning_rate, dataPlot):
    #
    ...
    #             # filename_out = 'C:\\Users\\Carlos\\Desktop\\output\\vtk_files\\LFBGSHole' + str(t)
    #             filename_out = folder_path + '\\output\\vtk_files\\LFBGSHole' + str(t)
    #
    #             z = np.array([0])
    #             U, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMises, F11, F12, F21, F22, P11, P12, P13, P21, P22, P23, P31, P32, P33 = self.evaluate_model(
    #                 dataPlot)
    #             des.save_WithHole(dataPlot.cpu().detach().numpy(), filename_out, U, S11, S12, S13, S22, S23, S33, E11,
    #                               E12, E13, E22, E23,
    #                               E33, SVonMises,
    ...
    # ...
    def train_modelADAM(self, data, neumannBC, dirichletBC, delaunayIntegration, tractionFreeCircle, iteration,
                        learning_rate, dataPlot):
        ...
        loss_fct: MSELoss = torch.nn.MSELoss()
        ...
        loss_array_energy = np.zeros(int(iteration / 100))
        loss_array_boundary = np.zeros(int(iteration / 100))
        loss_array_Pxx = np.zeros(int(iteration / 100))
        loss_array_Pxy = np.zeros(int(iteration / 100))
        loss_array_Pyx = np.zeros(int(iteration / 100))
        loss_array_Pyy = np.zeros(int(iteration / 100))
        loss_array_PBC = np.zeros(int(iteration / 100))
        loss_array_t1 = np.zeros(int(iteration / 100))
        loss_array_t2 = np.zeros(int(iteration / 100))
        loss_array_out1 = np.zeros(int(iteration / 100))
        loss_array_out2 = np.zeros(int(iteration / 100))
        loss_array_outin1 = np.zeros(int(iteration / 100))
        loss_array_outin2 = np.zeros(int(iteration / 100))

        total_plotting_loss = np.zeros(int(iteration / 100))
        ...
        for t in range(iteration):
            ...
            # Loss: domain training points loss (energy loss)
            energy_loss = internal - torch.sum(external)

            # Calculations for boundary loss (left & bottom)
            dir_u_pred_left = self.getU(dirBC_coordinates_left)[:, 0:1]
            bc_u_crit_left = self.loss_squared_sum(dir_u_pred_left, dirBC_values_left[:, 0:1])
            ...
            # u_pred is a concatenation of 6 variables: Ux, Uy, P11, P12, P21, P22
            duxdxy = grad(u_pred[:, 0].unsqueeze(1), x, torch.ones(x.size()[0], 1, device=dev), create_graph=True,
                          retain_graph=True)[0]
            duydxy = grad(u_pred[:, 1].unsqueeze(1), x, torch.ones(x.size()[0], 1, device=dev), create_graph=True,
                          retain_graph=True)[0]
            ...
            ### Calculation involving predicted P
            # partial derivatives of P_ij with respect to x_j --> used to calculate div(P)
            dP11dxy = grad(Pxx_out.unsqueeze(1), x, torch.ones(x.size()[0], 1, device=dev),
                           create_graph=True, retain_graph=True)[0]
            dP12dxy = grad(Pxy_out.unsqueeze(1), x, torch.ones(x.size()[0], 1, device=dev),
                           create_graph=True, retain_graph=True)[0]
            ...
            # calculate div(P) --> residual loss from predicted P
            out1 = dP11dxy[:, 0] + dP12dxy[:, 1]
            out2 = dP21dxy[:, 0] + dP22dxy[:, 1]
            ###

            ### Calculations involving P calculated from predicted u
            ...
            # calculate stress (from predicted u)
            P11 = mu * F11 + (lam * torch.log(detF) - mu) * invF11
            P12 = mu * F12 + (lam * torch.log(detF) - mu) * invF21
            P21 = mu * F21 + (lam * torch.log(detF) - mu) * invF12
            P22 = mu * F22 + (lam * torch.log(detF) - mu) * invF22
            ...
            # calculate div(P) --> residual loss from P calculated from predicted u
            outin1 = dP11indxy[:, 0] + dP12indxy[:, 1]
            outin2 = dP21indxy[:, 0] + dP22indxy[:, 1]
            ###

            # Loss terms for predicted P and P calculated from predicted u
            Pxx_loss = loss_fct(Pxx_out, P11[:, 0])
            Pxy_loss = loss_fct(Pxy_out, P12[:, 0])
            Pyx_loss = loss_fct(Pyx_out, P21[:, 0])
            Pyy_loss = loss_fct(Pyy_out, P22[:, 0])
            ...
            # Loss: traction boundary conditions loss (t loss)
            t1_loss = self.loss_sum(torch.pow(t1, 2))
            t2_loss = self.loss_sum(torch.pow(t2, 2))

            # Loss: Stress components boundary conditions loss (loss of P in boundaries)
            loss_sec1BCP11 = self.getLossStressesBoundary(pointsP11, valP11, 'P11')
            loss_sec1BCP12 = self.getLossStressesBoundary(pointsP12, valP12, 'P12')
            loss_sec1BCP21 = self.getLossStressesBoundary(pointsP21, valP21, 'P21')
            loss_sec1BCP22 = self.getLossStressesBoundary(pointsP22, valP22, 'P22')
            ...
            # Loss: displacement boundary conditions loss (boundary loss) (Dirichlet BC)
            boundary_loss = torch.sum(bc_u_crit_left + bc_u_crit_bottom)
            ...
            if t % 100 == 0:
                ...
                # assign as variable for plotting loss
                total_plotting_loss[int(t / 100)] = loss.item()

                # individual losses
                loss_array_energy[int(t / 100)] = energy_loss.item()
                loss_array_boundary[int(t / 100)] = boundary_loss.item()
                loss_array_Pxx[int(t / 100)] = Pxx_loss.item()
                loss_array_Pxy[int(t / 100)] = Pxy_loss.item()
                loss_array_Pyx[int(t / 100)] = Pyx_loss.item()
                loss_array_Pyy[int(t / 100)] = Pyy_loss.item()
                loss_array_PBC[int(t / 100)] = loscBC.item()
                loss_array_t1[int(t / 100)] = t1_loss.item()
                loss_array_t2[int(t / 100)] = t2_loss.item()
                loss_array_out1[int(t / 100)] = self.loss_sum(torch.pow(out1, 2)).item()
                loss_array_out2[int(t / 100)] = self.loss_sum(torch.pow(out2, 2)).item()
                loss_array_outin1[int(t / 100)] = self.loss_sum(torch.pow(outin1, 2)).item()
                loss_array_outin2[int(t / 100)] = self.loss_sum(torch.pow(outin2, 2)).item()
                # print("total_plotting_loss array:   ", total_plotting_loss)  # debug

            if t % 2000 == 0:
                # filename_out = './output/vtk_files/Hole' + str(t)
                # filename_out = 'C:\\Users\\Carlos\\Desktop\\output\\vtk_files\\Hole' + str(t)
                filename_out = folder_path + '\\output\\vtk_files\\Hole' + str(t)
        ...
        # save loss variables into excel file
        excel_data = {'total_loss': total_plotting_loss,
                      'energy': loss_array_energy,
                      'boundary': loss_array_boundary,
                      'Pxx': loss_array_Pxx,
                      'Pxy': loss_array_Pxy,
                      'Pyx': loss_array_Pyx,
                      'Pyy': loss_array_Pyy,
                      ...
                      't1': loss_array_t1,
                      't2': loss_array_t2,
                      'out1': loss_array_out1,
                      'out2': loss_array_out2,
                      'outin1': loss_array_outin1,
                      'outin2': loss_array_outin2}
        df = pd.DataFrame(excel_data, columns=None)
        writer = pd.ExcelWriter('ablation_losses.xlsx', engine='xlsxwriter')
    ...
    def getU(self, x):
        ...
        # Ux = x[:, 0].to(dev) * out[:, 0].to(dev) # try this out at some time?????????????????????????????????????????????????????????
        Uy = x[:, 1] * out[:, 1]
    ...
    def evaluate_model(self, dataPlot):
        ...
        if dim == 2:
            ...
            return U, np.float64(S11_pred), np.float64(S12_pred), np.float64(S13_pred), np.float64(
                S22_pred), np.float64(
                S23_pred), \
...
def run():
    ...
    dem = mDeepEnergyMethod(2, 6, nn.Tanh(), 60, 6, 2)

    # ----------------------------------------------------------------------
    #                   STEP 3: TRAINING MODEL
    # ----------------------------------------------------------------------
    dem.train_modelADAM(dom, boundary_neumann, boundary_dirichlet, delaunayIntegration, tractionFreeCircle,
                        iterationADAM, lrADAM, datatest)

    # iterationLFBGS = 100
    # lrLFBGS = 1e-3
    # dem.train_modelLFBGS(dom, boundary_neumann, boundary_dirichlet, delaunayIntegration, tractionFreeCircle,
    #                     iterationLFBGS, lrLFBGS, datatest)
